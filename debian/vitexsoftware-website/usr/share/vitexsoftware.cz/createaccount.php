...
if (isset($_POST) && \count($_POST)) {
    ...
    if (\strlen($email_address) < 5) {
        ...
        if (!$OUser->isEmail($email_address, true)) {
            ...
            if ($check_email[0]['total'] > 0) {
                $error = true;
                $OUser->addStatusMessage(
                    _('email address allready registred'),
                    'warning',
                );
            }
        }
    ...
    if ($oPage->MyDbLink->GetNumRows()) {
        ...
        $OUser->addStatusMessage(sprintf(
            _('The specified username %s is already in the database. Please choose another one.'),
            $login,
        ), 'warning');
    }
    ...
    if ($error === false) {
        ...
        if ($CustomerID) {
            ...
            $email = $oPage->addItem(new EaseMail(
                $NewOUser->getDataValue('email'),
                'Potvrzení',
            ));
            ...
            $email = $oPage->addItem(new \Ease\Mailer(
                SEND_ORDERS_TO,
...
$regForm = $oPage->column2->addItem(new \Ease\Html\Form(
    'create_account',
    'createaccount.php',
));
...
$account->addItem(new EaseLabeledPasswordStrongInput(
    'password',
    null,
    _('Password').' *',
));
$account->addItem(new EaseLabeledPasswordControlInput(
    'confirmation',
    null,
...
$personal->addItem(new EaseLabeledTextInput(
    'email_address',
    null,
...
$oPage->column1->addItem(new \Ease\Html\DivTag(
    'WelcomeHint',
    _('Letsgo'),
));
