...
/**
 * Phinx seed: import GitHub release notes as bilingual (EN + CS) blog articles.
 *
 * Requires:
 *   - gh CLI authenticated with GitHub
 *   - ANTHROPIC_API_KEY env var (optional; falls back to English if missing)
 *
 * Usage:
 *   vendor/bin/phinx seed:run -s ReleaseArticles
 *   ANTHROPIC_API_KEY=sk-... vendor/bin/phinx seed:run -s ReleaseArticles
 */
...
class ReleaseArticles extends AbstractSeed
{
    private const RELEASE_LIMIT = 20;
    ...
    public function run(): void
    {
        ...
        foreach ($repos as $repoPath => $_meta) {
            ...
            foreach ($releases as $rel) {
                ...
                if (!$enExists) {
                    $enHtml = $this->buildHtml($repoName, $tag, $logoUrl, $bodyHtml, $githubUrl, 'View release on GitHub →');
        ...
        $this->output->writeln("\n<info>Done.</info> Inserted: {$inserted} | Skipped: {$skipped} | Errors: {$errors}");
    }
